...
const ProductListing = ({ product }: { product: IProduct }) => {
  ...
  const displayPrice = (price: number | null | undefined, discount?: number) => {
    if (typeof price === "number") {
      if (discount && discount > 0) {
        ...
        return (
          <>
            <span className="line-through text-red-500">Rs. {price.toFixed(2)}</span>{" "}
  ...
  return (
    <div className="relative border rounded-md p-4 bg-white shadow-md group hover:shadow-lg transition-shadow duration-300">
      {/* Badges Container */}
      ...
      <button
        ...
        className="absolute top-4 right-4 p-2 bg-white rounded-full shadow-md hover:bg-red-100 transition duration-200 z-20"
        title={
